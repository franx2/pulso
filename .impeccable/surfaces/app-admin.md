...
THESIS: El escritorio de admin/encargado deja de ser el mismo stack de tarjetas centradas que usa un empleado en el celular — pasa a ser una consola operativa real: rail compacto expandible agrupado por sección + barra de contexto superior densa, refusing the mobile-card-shell-on-a-wide-screen default.
...
FIRST VIEWPORT: En escritorio (≥1024px) para rol ADMIN/ENCARGADO: rail fijo de 72px que se expande a 240px con hover o foco sin desplazar el contenido (marca arriba, grupos de nav rotulados al abrir, usuario/cerrar sesión abajo) + región principal con barra de contexto pegajosa (título + filtros/acciones de esa pantalla) + contenido en filas/tablas densas donde antes había grids de `Card`. Mobile y `/fichar` sin cambios: columna centrada + bottom-nav como está hoy.
