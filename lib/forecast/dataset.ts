...
/** Tope de seguridad por tramo, no el límite del histórico (ver TRAMO_DIAS). */
const MAX_PAGINAS = 400;

/**
 * La ventana se recorre en tramos por la misma razón que en fudoResumen.ts:
 * Fudo pagina de la venta más vieja a la más nueva, y al pedir un año de un
 * local grande el recorrido se cortaba justo en las ventas recientes. Acá el
 * daño era peor todavía: como las franjas se borran y se reinsertan, los días
 * que no llegaban quedaban directamente sin datos (Jumbo perdió 4 meses de
 * franjas y el pronóstico aprendía de un histórico mutilado).
 */
const TRAMO_DIAS = 45;
...
export async function construirDatasetLocal(
  ...
  const items = new Map<
    string,
    { cantidad: number; canceled: boolean | null; productoId?: string }
  >();
  ...
  const resCat = await fetch(
    `${API_URL}/product-categories?page[size]=200&fields[productCategory]=name`,
    { headers },
  );
  const catPorId = new Map<string, string>();
  if (resCat.ok) {
    const d = await resCat.json();
    for (const c of (d.data ?? []) as {
      id: string;
  ...
  for (let inicio = new Date(desde); inicio < hasta;) {
    const fin = new Date(
      Math.min(inicio.getTime() + TRAMO_DIAS * 86400000, hasta.getTime()),
    );
    let pagina = 1;
    for (;;) {
      const params = new URLSearchParams({
        "filter[createdAt]": `and(gte.${iso(inicio)},lte.${iso(fin)})`,
      ...
      if (!res.ok)
        throw new FudoError(
          `Fudo devolvió un error al listar ventas (HTTP ${res.status})`,
        );
      ...
      for (const inc of (data.included ?? []) as {
        ...
        if (inc.type === "Item") {
          ...
          categoriaDeProducto.set(
            inc.id,
            categoriaCanonica(catId ? catPorId.get(catId) : null),
          );
        }
      }
      ...
      for (const venta of lote) {
        ...
        for (const ref of venta.relationships?.items?.data ?? []) {
          ...
          const cat = item.productoId
            ? (categoriaDeProducto.get(item.productoId) ?? "otros")
            : "otros";
      ...
      pagina++;
      // Cortar en silencio acá borraba franjas que después no se reinsertaban.
  ...
  await db.demandaSlot.deleteMany({
    where: {
      localId,
      fecha: { gte: new Date(`${[...fechas].sort()[0]}T00:00:00.000Z`) },
    },
  });
  ...
  for (const l of locales) {
    try {
      ...
      out.push({
        local: l.nombre,
        ok: true,
        detalle: `${r.dias} días, ${r.slots} franjas, ${r.ventas} ventas`,
      });
    } catch (e) {
      out.push({
        local: l.nombre,
